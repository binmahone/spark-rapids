 ...
 */
case class VeloxFileSourceScanExec(originPlan: FileSourceScanExec
  ...
  require(originPlan.relation.fileFormat.getClass == classOf[ParquetFileFormat],
    "VeloxFileScan only supports ParquetFormat")
  ...
  @transient private lazy val nativePlan: SparkPlan = {
    VeloxBackendApis.overrideFileSourceScanExec(originPlan)
  }
  ...
  lazy val inputRDD: RDD[InternalRow] = {
    ...
    val cpuScanRDD = VeloxBackendApis.executeNativePlan(nativePlan)
    ...
    val embeddedMetrics = {
      ...
      veloxCommonMetrics.keys.foreach(key => mapBuilder += key -> allMetrics(key))
    ...
    new VeloxParquetScanRDD(cpuScanRDD,
      output,
      TargetSize(coalesceSizeGoal),
      embeddedMetrics,
      preloadedCapacity = rapidsConf.hybridParquetPreloadBatches)
  }
   ...
   */
  private val veloxCommonMetrics = Map[String, () => GpuMetric](
  ...
  override lazy val allMetrics: Map[String, GpuMetric] = {
    ...
    veloxCommonMetrics.foreach { case (key, generator) =>
      mapBuilder += key -> generator()
    }
    // NativeConverter specialized metrics
