...
abstract class MultiKudoTableVisitor<T, P, R> implements SchemaVisitor<T, P, R> {
  ...
  private final ArrayList<Deque<SliceInfo>> sliceInfoStack;
  ...
  protected MultiKudoTableVisitor(List<KudoTable> inputTables) {
    ...
    this.sliceInfoStack = new ArrayList<>();
    ...
    for (int i = 0; i < tables.size(); i++) {
      ...
      this.sliceInfoStack.add(new ArrayDeque<>(16));
      this.sliceInfoStack.get(i).add(new SliceInfo(header.getOffset(), header.getNumRows()));
  ...
  public T visitList(Schema listType, P preVisitResult, T childResult) {
    ...
    for (int tableIdx = 0; tableIdx < tables.size(); tableIdx++) {
      sliceInfoStack.get(tableIdx).removeLast();
    }
    ...
    for (int tableIdx = 0; tableIdx < tables.size(); tableIdx++) {
      ...
      if (sliceInfo.getRowCount() > 0) {
        if (updateSliceInfo) {
          ...
          sliceInfoStack.get(tableIdx).addLast(new SliceInfo(startOffset, rowCount));
        }
        ...
        if (updateSliceInfo) {
          sliceInfoStack.get(tableIdx).addLast(new SliceInfo(0, 0));
        }
      }
    }
