  ...
  val MULTITHREADED_SHUFFLE_ADAPTIVE_GPU_COMPRESSION =
    conf("spark.rapids.shuffle.multithreaded.adaptiveGpuCompression.enabled")
      .doc("Dynamically compress GPU-serialized Kudo shuffle partitions with nvCOMP Zstd when " +
        ...
        "the configured bound. After serialization, each eligible task requests the estimated " +
        "incremental compression bytes from the shared GPU memory semaphore without blocking. " +
        "A denied request follows the CPU Zstd path. " +
  ...
  val MULTITHREADED_SHUFFLE_ADAPTIVE_GPU_COMPRESSION_MAX_GPU_SEMAPHORE_WAITERS =
    conf("spark.rapids.shuffle.multithreaded.adaptiveGpuCompression.maxGpuSemaphoreWaiters")
      .doc("Maximum observed GPU semaphore waiter count that permits a task to propose adaptive " +
        "GPU shuffle compression. Final memory admission is owned by the shared GPU semaphore.")
      .internal()
      .startupOnly()
      .integerConf
...
val SHUFFLE_COMPRESSION_LZ4_CHUNK_SIZE = conf("spark.rapids.shuffle.compression.lz4.chunkSize")
  ...
  val SHUFFLE_MULTITHREADED_READER_ADAPTIVE_GPU_CONCURRENCY_MULTIPLIER =
    conf("spark.rapids.shuffle.multiThreaded.reader.adaptiveAdmission.gpuConcurrencyMultiplier")
      .doc("Multiplier applied to estimated dynamic GPU concurrency to derive the reader " +
        "admission target.")
      .internal()
      .startupOnly()
      .doubleConf
  ...
  val SHUFFLE_MULTITHREADED_READER_ADAPTIVE_STABLE_TARGET_WINDOWS =
    conf("spark.rapids.shuffle.multiThreaded.reader.adaptiveAdmission.stableTargetWindows")
      .doc("Consecutive decision windows that must report the same GPU-derived target before " +
        "reader admission moves one permit toward it.")
      .internal()
      .startupOnly()
      .integerConf
