...
import com.nvidia.spark.rapids.PriorityAwareFileReaderThreadPool

/**
 * A CompletionService implementation for PriorityAwareFileReaderThreadPool that provides
 * priority-aware async task execution and completion handling. This service extends the standard
 * Java CompletionService pattern by accepting only AsyncRunner instances, wrapping them in
 * CompletionFutureTask for completion tracking, and maintaining completion order through a
 * completion queue for efficient result polling.
 *
 * This implementation uses pool.execute() instead of pool.submit() to preserve the
 * RapidsFutureTask wrapper, which is required for the priority queue comparator to work correctly.
 *
 * @param pool the PriorityAwareFileReaderThreadPool to use for task execution
 * @tparam V the result type of the AsyncRunners
 */
class PriorityAwareCompletionService[V](
    pool: PriorityAwareFileReaderThreadPool) extends CompletionService[AsyncResult[V]] {
  ...
  override def submit(task: Callable[AsyncResult[V]]): Future[AsyncResult[V]] = {
    task match {
      case runner: AsyncRunner[V] =>
        ...
        // Use execute() instead of submit() to preserve the RapidsFutureTask wrapper.
        // The priority queue comparator expects RapidsFutureTask, not FutureTask wrapper.
