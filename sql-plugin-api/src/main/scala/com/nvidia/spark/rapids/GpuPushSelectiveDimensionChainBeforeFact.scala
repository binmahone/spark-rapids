 ...
 */
case class GpuPushSelectiveDimensionChainBeforeFact(spark: SparkSession)
  ...
  override def apply(plan: LogicalPlan): LogicalPlan = {
    ...
    if (firstApplyLog.compareAndSet(false, true)) {
      logDebug(
        s"GpuPushSelectiveDimensionChainBeforeFact: enabled root=${plan.nodeName} " +
      ...
      case p @ Project(projectList, child) if projectList.forall(_.deterministic) =>
        extractInnerJoinCluster(child) match {
          case Some((input, conditions)) if input.length >= 4 && input.length <= maxClusterItems =>
            if (input.length >= 6) {
              logDebug(
                "GpuPushSelectiveDimensionChainBeforeFact: candidate cluster " +
            ...
            tryReorder(child, input, conditions, requiredOutput) match {
              case Some(rewritten) if !rewritten.fastEquals(child) =>
                logWarning(
                  "GpuPushSelectiveDimensionChainBeforeFact: rewrote project-wrapped cluster " +
          ...
          case Some((input, conditions)) if input.length >= 4 && input.length <= maxClusterItems =>
            ...
            tryReorder(p, input, conditions, p.outputSet) match {
              case Some(rewritten) if !rewritten.fastEquals(p) =>
                logWarning(
                  "GpuPushSelectiveDimensionChainBeforeFact: rewrote cluster " +
  ...
  private def registerPostCboPass(): Unit = {
    ...
    experimental.synchronized {
      if (!experimental.extraOptimizations.exists(
          _.isInstanceOf[GpuPushSelectiveDimensionChainBeforeFact])) {
        experimental.extraOptimizations = experimental.extraOptimizations :+ this
        logDebug(
          "GpuPushSelectiveDimensionChainBeforeFact: registered in " +
            "spark.experimental.extraOptimizations for the post-CBO pass")
      }
    }
  }
      ...
      v =>
        pruningEdge(chainDim, v, uf, seed).flatMap {
          case (chainAttr, victimAttr) =>
            ...
            // executor peer (worst-case broadcast) and credits only the victim bytes avoided at
            // those
  ...
  private def extendedChainCostJustifiesRewrite(
    ...
    val peerCount = BigInt(positiveLongConf("spark.executor.instances", 1L))
    ...
    if (worthwhile) {
      logInfo(
        "GpuPushSelectiveDimensionChainBeforeFact: admitted extended chain by cost " +
