 ...
 * GpuBroadcastReplicatePartitioning. Each consumer task reads its assigned
 * shuffle partition — which under replicate-broadcast contains a full copy
 * of the build side — and uses it directly as the BHJ build batch.
 *
...
case class GpuShuffleBroadcastHashJoinExec(
  ...
  // The build side is fed by a GpuShuffleExchangeExec writing
  // GpuBroadcastReplicatePartitioning. The shuffle is already enforcing the
  // "every consumer sees the full build" contract at the partitioning level,
  // so this operator does NOT need to declare BroadcastDistribution on its
  // child; UnspecifiedDistribution is enough.
  ...
  override def internalDoExecuteColumnar(): RDD[ColumnarBatch] = {
    ...
    // Collect all shuffleIds that need "no per-block release" treatment:
    //   1. Our own build shuffle (every consumer task reads the same blocks)
    //   2. Any GpuShuffleExchangeExec referenced by a ReusedExchangeExec
    //      anywhere in our subtree (multi-consumer regular shuffle)
    val multiConsumerIds = scala.collection.mutable.Buffer[Int](buildShuffleId)
    Seq(buildPlan, streamedPlan).foreach { sub =>
      sub.foreach {
        case ReusedExchangeExec(_, gpu: GpuShuffleExchangeExec) =>
          multiConsumerIds += gpu.shuffleDependencyColumnar.shuffleId
        case _ =>
      }
    }

    // Tag these shuffles via a Spark local property that the executor side
    // reads in RapidsShuffleInternalManagerBase.registerGpuShuffle (calls
    // catalog.markNoPerBlockRelease). Property is additive — multiple BHJ
    // rewrites in the same query all contribute their shuffleIds.
    val key = RapidsShuffleInternalManagerBase.NATIVE_BROADCAST_SHUFFLE_IDS_KEY
    val existing = Option(sparkContext.getLocalProperty(key)).getOrElse("")
    val combined = (existing.split(",").filter(_.nonEmpty) ++
      multiConsumerIds.map(_.toString)).distinct.mkString(",")
    sparkContext.setLocalProperty(key, combined)
