...
LOG_DIR="$ROOT/v042-executor-global-writer-limit-validation/build-logs"
...
mvn "${COMMON_ARGS[@]}" \
  -pl sql-plugin -am \
  -DwildcardSuites=org.apache.spark.sql.rapids.SharedBytesInFlightLimiterSuite,org.apache.spark.sql.rapids.ReaderTaskAdmissionGateSuite \
  package \
  2>&1 | tee "$LOG_DIR/${TIMESTAMP}-admission-tests.log"
